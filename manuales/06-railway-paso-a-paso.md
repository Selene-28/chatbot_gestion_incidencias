...
| Panel login: JSON `Ocurrió un error interno` o chat «No se pudo conectar» | `/healthz` dice `"db":"down"`. Si `db_error` menciona `Name or service not known`, la caja **mysql** no se llama exactamente `mysql` o no hay red privada. En `DB_URL` usa el host privado que muestra **mysql → Settings → Networking** (o `${{mysql.RAILWAY_PRIVATE_DOMAIN}}`). |
